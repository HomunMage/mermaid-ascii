...
class GraphIR:
    ...
    Node data is stored as node attributes under key 'data'.
    Edge data is stored as edge attributes under key 'data'.
    """
    ...
    def from_ast(cls, ast_graph: ast.Graph) -> GraphIR:
        ...
        # Collect subgraph names (for subgraph-as-edge-endpoint detection).
        ...
        # Process subgraphs — collect members for rendering.
        for sg in ast_graph.subgraphs:
            _collect_subgraph(sg, digraph, subgraph_members)

        # Add top-level edges. If an endpoint doesn't exist yet, create a
        # placeholder node — collapse_subgraphs() will redirect it later.
    ...
    def adjacency_list(self) -> list[tuple[str, list[str]]]:
        """Returns sorted adjacency list: node id → sorted outgoing neighbor ids."""
...
def _add_node_if_absent(
    ...
    """Add a node to digraph if not already present."""
...
def _ensure_node(digraph: nx.DiGraph, node_id: str) -> None:
    """Ensure a node with the given id exists. Creates a placeholder if missing."""
...
def _collect_subgraph(
    ...
    """Recursively collect nodes from a subgraph into the DiGraph."""
    member_ids: list[str] = []
